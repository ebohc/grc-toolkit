...
**If you're an auditor or assessor reviewing evidence:** the Domain Detail section states which framework's version of a control is the strictest, so you know which evidence set to request first, usually the superset.
...
This is also where the most common real-world PCI scope mistake happens, and it's worth naming because it catches experienced teams as often as new ones. A merchant outsources checkout entirely to a compliant payment provider, qualifies for the simplified SAQ A, and reasonably assumes the scope question is closed. It usually isn't. The PCI Council is explicit that outsourcing reduces scope, it does not remove responsibility: the merchant still has to validate its own compliance, maintain the Req. 12.8.2 written agreement, and monitor the provider's status at least annually under Req. 12.8.4. The scope boundary breaks in a specific, ordinary way: a customer calls in with a billing dispute or wants to place an order by phone, and someone in the back office types the card number into a form to process it. That phone line and that workstation just entered the cardholder data environment. Nobody approved it, nobody built a control for it, and SAQ A no longer applies the moment it happens. The fix isn't more paperwork on the outsourced channel. It's mapping every channel data can actually move through, including the ones nobody designed on purpose, not just the one the SAQ was scoped against.
